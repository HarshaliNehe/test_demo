...
session_start();//get the session
 if(isset($_GET['id'])){//check if we get id from a get
 	...
 	if(isset($_SESSION['id'])){//check if session exsist
		...
		$conn=mysqli_connect('localhost','root','','news');//make a connection to the database
	...
	echo "<script language='Javascript'>";
  	...
  	$delete=mysqli_query($conn,"DELETE FROM publication_info where id='$id'");//delete post with the get id
  	echo "<script language='Javascript'>";
